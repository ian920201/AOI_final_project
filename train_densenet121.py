...
model.classifier = nn.Linear(num_ftrs, 31)
...
def main():
    clear_csv("./result_data/result_densenet121.csv")
    n = int(sys.argv[1])
    for epoch in range(1, n + 1):
        print("epoch: " + str(epoch))
        train_loss = train()
        acc = test()
        write_csv("./result_data/result_densenet121.csv", epoch, acc)
        print(f"Epoch [{epoch}/{n}] - Loss: {train_loss:.4f} - Accuracy: {acc:.4f}")

    torch.save(model.state_dict(), "./pth/face_densenet121.pth")
